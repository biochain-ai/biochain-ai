...
// # Init
//
// Inizialize chaincode
//
//	TODO: Add a check to see if the REQUESTCOUNTER is already present.
//	 If present DO NOT add a new one
...
func (c *Chaincode) removeData(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// Check if the one how asked for deletion belongs to the same organization
	// of the owner
	...
	var creatorMSPID string
	var owner_split []string

	// retrieve creator MSPID
	creatorMSPID = getMSPID(stub)
	// retrieve owner MSPID
	owner_split = strings.Split(data.Owner, "@")
	...
	err = stub.DelPrivateData(collectionName, removeDataInput.Name)
	if err != nil {
...
func (c *Chaincode) viewPersonalData(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for resultIterator.HasNext() {
		...
		if strings.Contains(queryResponse.Key, "DATA") {
			...
			// Check ORGANIZATION LEVEL	authorization
			// retrieve creator MSPID
			creatorMSPID = getMSPID(stub)
			var owner_split []string

			// retrieve data owner MSPID
			owner_split = strings.Split(dataElement.Owner, "@")

			if creatorMSPID == owner_split[1] {
				// Add a comma before array member
				if bArrayMemberAlreadyWritten {
					buffer.WriteString(",")
				}
...
func (c *Chaincode) viewRequests(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for resultIteratorRequests.HasNext() {
		...
		if strings.Contains(queryResponse.Key, "REQUEST") &&
			...
			creatorMSPID = getMSPID(stub)
			owner_split = strings.Split(requestElement.Owner, "@")

			// Check the owner (ORGANIZATION LEVEL)
			if creatorMSPID == owner_split[1] {
				// Add a comma before array member
				if bArrayMemberAlreadyWritten {
					buffer.WriteString(",")
...
func (c *Chaincode) acceptRequest(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// ORGANIZATION LEVEL
	// Check if the caller belowngs to the org that owns the requested resource
	var creatorMSPID string
	var owner_split []string

	creatorMSPID = getMSPID(stub)
	owner_split = strings.Split(requestToAccept.Owner, "@")
...
func (c *Chaincode) denyRequest(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	creatorMSPID = getMSPID(stub)
	owner_split = strings.Split(requestToReject.Owner, "@")
...
// # getFullname
//
// This function returns a string that represents the creator fullname of the
// transaction (commonName@MSPID)
func getFullname(stub shim.ChaincodeStubInterface) (creator string) {
	// Id, _ := cid.GetID(stub)
	X509, _ := cid.GetX509Certificate(stub)
	CN := X509.Subject.CommonName
	CN_split := strings.Split(CN, "@")
	MSPID, _ := cid.GetMSPID(stub)
	creator = CN_split[0] + "@" + MSPID
	return creator
}

// # getCommonName
//
// This function returns a string that represents the creator name of the
// transaction (commonName)
func getCommonName(stub shim.ChaincodeStubInterface) (creator string) {
	X509, _ := cid.GetX509Certificate(stub)
	CN := X509.Subject.CommonName
	CN_split := strings.Split(CN, "@")
	return CN_split[0]
...
// # getMSPID
//
// This function returns a string that represents the creator organization of
// the transaction (MSPID)
func getMSPID(stub shim.ChaincodeStubInterface) (creator string) {
	MSPID, _ := cid.GetMSPID(stub)
	return MSPID
}
